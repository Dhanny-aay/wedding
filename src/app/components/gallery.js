...
const Gallery = () => {
  ...
  const handleLoadMore = () => {
    ...
    setDisplayLimit((prevLimit) => prevLimit + ITEMS_PER_LOAD);
  };
  ...
  const displayedMoments = momentsData.slice(0, displayLimit);
  ...
  const SkeletonCard = ({ tall }) => (
    ...
    >
      ...
      <div className="absolute top-4 right-4 bg-gray-300 w-8 h-8 md:w-10 md:h-10 rounded-[4px] animate-pulse"></div>
    </div>
  );
  ...
  const EmptyState = () => (
    <div className="w-full max-w-[600px] mx-auto text-center py-16">
      ...
      <button
        ...
        className="bg-primary py-3 px-6 text-white rounded-[32px] text-sm md:text-base hover:opacity-90 transition-opacity"
      >
        Share Your Memory
      </button>
    </div>
  ...
  const GalleryImage = ({ image, tall }) => (
    ...
    >
      <img
        src={image.src}
        alt={image.alt}
        className="w-full h-full object-cover transition-transform duration-300 relative"
      />
      <button
        onClick={() => openModal(image)}
        className="absolute top-4 right-4 bg-white md:w-10 md:h-10 p-1 rounded-[4px] flex items-center justify-center cursor-pointer"
      >
  ...
  return (
    <>
      <div className="py-[100px] px-4 md:px-12 lg:px-40 flex flex-col items-center bg-[#FDF6F8]">
        <p className="text-primary text-sm tracking-[5px] uppercase">
          Our Gallery
        </p>
        <h2 className="mt-4 font-alexBrush text-[#1b1b1b] text-[60px] leading-[60px]">
          Our Memories
        </h2>

        <button
          onClick={openShareModal}
          className="mt-6 bg-primary py-4 w-full max-w-[500px] flex items-center justify-center space-x-1.5 text-sm md:text-lg text-white rounded-[32px]"
        >
        ...
        <div className="mt-6 flex items-center gap-3.5">
          <button className="bg-primary rounded-[50px] px-[17px] py-[7px] text-white text-sm md:text-lg">
            All
          </button>
          <button className="border-2 border-[#008080] rounded-[50px] px-[17px] py-[7px] text-primary text-sm md:text-lg">
            Pre Wedding
          </button>
          <button className="border-2 border-[#008080] rounded-[50px] px-[17px] py-[7px] text-primary text-sm md:text-lg">
            Wedding
        ...
        <div className="w-full max-w-[1440px] mt-12 space-y-8">
          {isLoading ? (
            ...
            chunkArray(displayedMoments, 6).map((group, i) => (
          ...
          {isFetchingNextPage && <SkeletonGroup />}
        </div>

        {!isLoading && displayedMoments.length > 0 && (
          <div className="mt-[50px] flex gap-4">
            {(hasNextPage || displayedMoments.length < momentsData.length) && (
              <button
                onClick={handleLoadMore}
                disabled={isFetchingNextPage}
                className="bg-primary rounded-[50px] px-[24px] py-[16px] text-white text-sm md:text-base md:leading-[20px] disabled:opacity-50"
              >
            ...
            {displayLimit > ITEMS_PER_LOAD && (
              <button
                ...
                className="border-2 border-primary rounded-[50px] px-[24px] py-[16px] text-primary text-sm md:text-base md:leading-[20px]"
              >
                Show Less
              </button>
            )}
